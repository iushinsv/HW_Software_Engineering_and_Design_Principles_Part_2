from link: https://docs.spring.io/spring-boot/reference/actuator/metrics.html#actuator.metrics
...
SSL Bundle Metrics
Spring Boot Actuator publishes expiry metrics about SSL bundles. The metric ssl.chain.expiry gauges the expiry date of each certificate chain in seconds. This number will be negative if the chain has already expired. This metric is tagged with the following information:
...
Redis Metrics
Auto-configuration registers a MicrometerTracing for the auto-configured LettuceConnectionFactory. For more detail, see the Observability section of the Lettuce documentation.
